...
```python
def list_chunks_for_file(filename: str, index_path: str = INDEX_FILE) -> list[str]:
    """Show all indexed chunks for a specific document."""
    index = load_index(index_path)
    return [e["text"] for e in index if e["filename"] == filename]


def delete_index(index_path: str = INDEX_FILE) -> str:
    """Remove the index file so it can be rebuilt cleanly."""
    p = Path(index_path)
    if p.exists():
        p.unlink()
        return f"Deleted {index_path}"
    return f"{index_path} does not exist"


def search_with_scores(query: str, index_path: str = INDEX_FILE, top_k: int = 5) -> list[dict]:
    """
    Same as retrieve_chunks but returns more results and shows full chunk text.
    Useful for debugging retrieval quality.
    """
    return retrieve_chunks(query, index_path=index_path, top_k=top_k)
```
...
| **Index** | A stored collection of chunks + their embeddings |
| **Retrieval** | Finding chunks most similar to the user's query |
| **Context window** | The text passed to the LLM along with the question |
| **Grounded answer** | An answer based only on retrieved content, not model memory |
...
- [x] Loaded and chunked plain-text documents from a local folder
- [x] Embedded each chunk using `nomic-embed-text` running locally via Ollama
- [x] Saved a vector index to disk (`index.json`) for reuse
- [x] Implemented cosine similarity search to retrieve relevant chunks
- [x] Built a full RAG pipeline: retrieve → inject context → generate grounded answer
- [x] Built an interactive terminal CLI in `cli.py`
- [x] Built a browser UI in `app.py` using Flask
- [x] Tested both answerable and unanswerable queries

---

*Workshop — RAG System + Ollama · April 2026*
